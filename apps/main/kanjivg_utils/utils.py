...
class SvgFileInfo:
    def __init__(self, file, dir):
        ...
        if file[-4:].lower() != ".svg":
            raise Exception("File should have .svg exension. (%s)" %
                            (str(self.path)))
        parts = (file[:-4]).split('-')
        if len(parts) == 2:
            ...
            raise Exception(
                "File should have at most 2 parts separated by a dash. (%s)" % (str(file)))
        self.id = parts[0]
        if self.id != canonicalId(self.id):
            raise Exception(
                "File name not in canonical format (%s)" % (str(self.path)))
    ...
    def read(self, SVGHandler=None):
        ...
        if len(parsed) != 1:
            raise Exception(
                "File does not contain 1 kanji entry. (%s)" % (self.path))
        return parsed[0]
...
def FindSvg(arg):
    id = canonicalId(arg)
    url = os.path.join(BASE_DIR, 'main', 'kanjivg_utils', 'kanji')
    for f in listSvgFiles(url):
        if f.id == id:
            custom_kanji_path = f.read().code + ".svg"
    return custom_kanji_path
