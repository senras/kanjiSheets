...
class StrokeGr:
    ...
    def toSVG(self, out, rootId, groupCpt=[0], strCpt=[1], indent=0):
        gid = rootId
        if groupCpt[0] != 0:
            gid += "-g" + str(groupCpt[0])
        groupCpt[0] += 1

        idString = ' id="kvg:%s"' % (gid)
        eltString = ""
        if self.element:
            eltString = ' kvg:element="%s"' % (self.element)
        variantString = ""
        if self.variant:
            variantString = ' kvg:variant="true"'
        partialString = ""
        if self.partial:
            partialString = ' kvg:partial="true"'
        origString = ""
        if self.original:
            origString = ' kvg:original="%s"' % (self.original)
        partString = ""
        if self.part:
            partString = ' kvg:part="%d"' % (self.part)
        numberString = ""
        if self.number:
            numberString = ' kvg:number="%d"' % (self.number)
        tradFormString = ""
        if self.tradForm:
            tradFormString = ' kvg:tradForm="true"'
        radicalFormString = ""
        if self.radicalForm:
            radicalFormString = ' kvg:radicalForm="true"'
        posString = ""
        if self.position:
            posString = ' kvg:position="%s"' % (self.position)
        radString = ""
        if self.radical:
            radString = ' kvg:radical="%s"' % (self.radical)
        phonString = ""
        if self.phon:
            phonString = ' kvg:phon="%s"' % (self.phon)
        out.write("\t" * indent + '<g%s%s%s%s%s%s%s%s%s%s%s%s>\n' % (idString, eltString, partString, numberString,
                  variantString, origString, partialString, tradFormString, radicalFormString, posString, radString, phonString))
        ...
        for child in self.childs:
            if isinstance(child, StrokeGr):
                ...
                if not found or recursive:
                    ...
                    if found:
                        newLevel += 1
                    childsComp += child.components(simplified,
                                                   recursive, newLevel)
        if recursive and not len(ret) == 0:
    ...
    def simplify(self):
        ...
        if len(self.childs) == 1 and isinstance(self.childs[0], StrokeGr):
            ...
            if child.tradForm and self.tradForm and child.tradForm != self.tradForm:
                return
            if child.radicalForm and self.radicalForm and child.radicalForm != self.radicalForm:
                return
...
class KanjisHandler(BasicHandler):
    ...
    def handle_start_kanji(self, attrs):
        ...
        if idType != "kvg:kanji":
            raise Exception(
                "Each kanji should have id formatted as kvg:kanji_XXXXX.")
    ...
    def handle_start_g(self, attrs):
        ...
        if group.number:
            if not group.part:
                print("%s: Number specified, but part missing" %
                      (self.kanji.kId()))
            ...
            if group.part > 1:
                if (group.element + str(group.number)) not in self.compCpt:
                    ...
                    print("%s: Incorrectly numbered group" %
                          (self.kanji.kId()))
        ...
        # increase correctly
            ...
            if group.part > 1:
                if group.element not in self.compCpt:
                    print("%s: Incorrectly started multi-part group" %
                          (self.kanji.kId()))
                elif self.compCpt[group.element] != group.part - 1:
                    print("%s: Incorrectly splitted multi-part group" %
                          (self.kanji.kId()))
    ...
    def handle_start_g(self, attrs):
        ...
        if len(self.groups) == 0:
            ...
            if idType == "kvg:StrokePaths":
                ...
                raise Exception("Invalid root group id type (%s)" %
                                (str(attrs["id"]),))
        ...
        if group.number:
            ...
            # The group must not exist
                ...
                if (group.element + str(group.number)) in self.compCpt:
                    ...
                    print("%s: Duplicate numbered group" %
                          (self.currentKanji.kId()))
            self.compCpt[group.element + str(group.number)] = group.part
        # No number, just a part - groups restart with part 1, otherwise must
        # increase correctly
...
def realchr(i):
    if i < 0x10000:
        ...
        return chr(((i - 0x10000) >> 10) + 0xD800) + chr(0xDC00 + (i & 0x3ff))
